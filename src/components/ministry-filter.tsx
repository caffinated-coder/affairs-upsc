...
export function MinistryFilter({ lang, filter, onChange, countsByName }: Props) {
  ...
  const total = [...countsByName.values()].reduce((a, b) => a + b, 0);
  const rows = useMemo(() => {
  ...
  return (
    <div className="rounded-lg border border-line bg-surface p-3">
      ...
      <div className="relative mb-2">
        ...
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search ministry" className="h-9 pl-7 text-sm" />
      </div>
        ...
        {GROUP_ORDER.map((group) => {
          ...
          return (
            <div key={group} className="mb-2">
              <p className="px-1 py-1 text-[10px] uppercase tracking-wider text-faint">{GROUP_LABEL[group].en}</p>
              {gRows.map(({ m, count }) => {
